...
## 2026-07-02 — Post-review hardening: numeric depth backstop + drop dead `description`

**Phase:** implement (post-review)

**Decision:** Acting on the 2026-07-02 adversarial review (`docs/feature/task-tool/review-2026-07-02-task-tool-review.md`), applied the review's fixes. Two carry design weight and **supersede earlier decisions/spec text**:

1. **Numeric recursion backstop added (review D1) — reverses the R2 deferral.** A numeric depth guard now complements the structural bound. `agentLoop` seeds `context.depth` (0 at the top level) from a new **optional `RunOptions.depth`** (documented `@internal`); `createTaskTool` gains an optional **`maxDepth` (default 1)**, drives each child at `depth + 1`, and refuses to spawn when `context.depth >= maxDepth` — returning the new model-facing microcopy `"Sub-agent depth limit reached (maxDepth=N); refusing to spawn a nested sub-agent."` (`isError:true`, zero child tokens). The structural bound (`resolveChild` omits `task`) remains the primary protection.

2. **Required `description` input dropped (review D4).** The `task` schema's required `description` field (labeled "for logging") was never read, forwarded, logged, or surfaced — a dead contract. It is removed from `taskInputSchema`. The model no longer emits it.

Also applied (no design weight): I1 `try/catch` around the child-driver (pre-stream child rejection now maps to the `"Sub-agent failed: "` microcopy and tears the child generator down); I2 clearing `context.reportUsage`/`emitEvent` after each tool batch; I3 corrected the misleading loop fold comment; nits — `childCtrl` removed (the child's own `Agent.run` re-wraps the signal, so `context.signal` alone gives the E3 cascade + isolation), `toolCallId` JSDoc corrected, `agent.ts` cancel comment softened (D3), `prompt` gets `.min(1)`, and `mapChildTerminalToResult` gets an `assertNever` default. D2 (real-time child events), D3 (break ≠ cancel), and I3's observability gap are **documented, not re-implemented** (see `known-issues.md`).

**Rationale:** The user explicitly opted into building the depth backstop rather than documenting the footgun. A real numeric guard fundamentally requires the child's loop to know its depth, which no channel provided while `Agent.run` stayed fully closed — so the closed-`Agent.run` decision (2026-06-27, reaffirmed in this feature) is knowingly relaxed by the **minimum** necessary: a single optional field, threaded by the `task` tool itself (not by hosts), so top-level consumers are unaffected and hosts write no new code. `AsyncLocalStorage` was rejected (fragile across async-generator resumption; needs a restricted `node:async_hooks` import in core). `maxDepth` default 1 matches v1's depth-1 posture, so correct hosts (which omit `task`) never observe it and the misconfigured-host footgun is caught by default. Dropping `description` (vs. surfacing it) was the user's choice — the field had no honest channel and the forcing-function value did not justify a mandatory model-facing field.

**Consequences:** `RunOptions`, `ToolCallContext`, and `LoopParams` each gain an optional `depth`; `CreateTaskToolOptions` gains optional `maxDepth`. No new public export (all ride on already-exported types). The model-facing `task` schema loses `description`; the microcopy set gains the depth-limit string. The R2/known-issues "numeric depth guard deferred" item is now **resolved**; D2/D3/I3 are recorded as accepted limitations. The 2026-07-01 engineering spec's Microcopy (required `description`) and its R2 deferral are historical — this entry is the living record (spec carries a Rev-4 pointer). Tests: `task-tool.test.ts` gains T-cov-1 (abort-mid-flight cascade) and T-cov-2a/b/c (depth backstop); the field-validation test is rewritten for prompt-only. Suite: 318 tests green, `tsc` clean, eslint clean.
